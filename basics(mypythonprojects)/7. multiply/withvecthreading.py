...
def timemults(func1, func2, func3):
    ...
    r = np.arange(size)
    plt.title("blue:func1, red:func2, green:func3")
    plt.plot(r, list1, color="blue")
    plt.plot(r, list2, color="red")
    plt.plot(r, list3, color="green")
    plt.show()
...
def asyncmult(m1, m2):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for i in range(m1.shape[0]):
            for j in range(m2.shape[1]):
                f = executor.submit(multvecs, (m1[i], m2[:, j], (i, j), executor))
                threads.append(f)
...
def multvecs(inp):
    v1, v2, index, executor = inp
    sum = 0
    threads = []
    for i in range(v1.shape[0]):
        f = executor.submit(mult, (v1[i], v2[i]))
        threads.append(f)
...
m1 = np.random.rand(9, 10) * 10
m2 = np.random.rand(10, 11) * 10
